...
class BudgetService:
    ...
    async def create_or_update_budget(self, obj_in: BudgetCreate, user_id: uuid.UUID = settings.DEFAULT_USER_ID) -> BudgetResponse:
        ...
        cat_name = category.name if obj_in.category_id and 'category' in locals() else ("Overall Monthly Budget" if not obj_in.category_id else None)
    ...
    async def get_budget_status(
        ...
        # Fetch budget rule
        budget = await self.repo.get_by_category_id(category_id, user_id=user_id)
